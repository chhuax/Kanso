...
`⇧⌘R` (`Ctrl+Shift+R` elsewhere) opens a search over everything remembered, across every tab and host, with how often and how long ago each command was last run. The arrows walk it, `Enter` puts the chosen command on the line — unsubmitted, so it can be read and edited before it runs — and `Esc` closes it. The shell keeps plain `Ctrl+R` and the reverse search that goes with it. **Edit → Clear Command History…** clears the recorded history.
...
| `⇧⌘R` | `Ctrl+Shift+R` | Search the remembered command history |
