...
- **Status bar** (bottom): the active session's terminal size, protocol and clock — and, for a local shell, the directory it is in and the git branch on it, both refreshed as you work. A path too long for the bar keeps its tail (`…/sandbox/api`), since that is the half that says where you are.
