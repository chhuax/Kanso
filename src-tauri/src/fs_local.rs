...
/// usable without restarting ZenTerm. Other platforms do a single attempt.
...
/// drag, mirroring `remote_edit`'s layout under the same `ZenTerm` temp
/// directory.
fn drag_root() -> PathBuf {
    std::env::temp_dir().join("ZenTerm").join("drag")
}
...
pub fn clean_drag_staging() {
    if let Err(error) = std::fs::remove_dir_all(drag_root()) {
        if error.kind() != io::ErrorKind::NotFound {
            eprintln!("ZenTerm: could not clear the drag staging folder: {error}");
        }
    }
}
