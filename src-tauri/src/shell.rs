...
//!
//! Nothing in the user's home is touched, and only the shells Kanso starts
...
const ZSH_HOOK: &str = r#"# Kanso: where the shell is, as the chips the coding CLIs print, after the
...
kanso_precmd() {
  ...
  [[ $directory_is_shown == no ]] &&
    KANSO_CHIPS="${chip} ${text}${where} ${off}"
  if [[ -n $branch ]]; then
    [[ -n $KANSO_CHIPS ]] && KANSO_CHIPS+=" "
    KANSO_CHIPS+="${chip} ${green}${fork} ${branch} ${off}"
  fi
  ...
  if [[ -z $KANSO_CHIPS ]]; then
    PROMPT=$KANSO_BASE
  elif [[ $KANSO_BASE == '%'[nNmM]* ]]; then
    PROMPT="${KANSO_BASE[1,2]} ${KANSO_CHIPS}${KANSO_BASE[3,-1]}"
  else
    PROMPT="${KANSO_CHIPS} ${KANSO_BASE}"
  fi
...
mod tests {
    ...
    fn the_hook_reads_a_branch_and_prints_the_line() {
        ...
        let root = std::env::temp_dir().join(format!("kanso-hook-{}", uuid::Uuid::new_v4()));
        ...
        let Some(branch) = run(&repo, "kanso_branch $PWD") else {
        ...
        assert_eq!(
            run(&deep, "kanso_branch $PWD").as_deref(),
            Some("feature/x")
        );
        ...
        let with_dir = run(
            &deep,
            "PROMPT='%n %~ %# '; KANSO_USER_PROMPT=$PROMPT; kanso_precmd; print -rn -- \"$PROMPT\"; print -r -- '|'",
        )
        .expect("zsh");
        assert!(
            with_dir.contains(".../"),
        ...
        let literal = run(
            &deep,
            "PROMPT='%n $PWD %# '; KANSO_USER_PROMPT=$PROMPT; kanso_precmd; print -r -- \"$PROMPT\"",
        )
        .expect("zsh");
        assert!(
            !literal.contains(".../"),
        ...
        let tail = run(
            &deep,
            "PROMPT='%n@%m %1~ %# '; KANSO_USER_PROMPT=$PROMPT; kanso_precmd; print -r -- \"$PROMPT\"",
        )
        .expect("zsh");
        assert!(
            tail.contains(".../"),
        ...
        let numbered = run(
            &deep,
            "PROMPT='%n %2~ %# '; KANSO_USER_PROMPT=$PROMPT; kanso_precmd; print -r -- \"$PROMPT\"",
        )
        .expect("zsh");
        assert!(
            numbered.contains(".../"),
        ...
        let bare = run(
            &deep,
            "PROMPT='$ '; KANSO_USER_PROMPT=$PROMPT; kanso_precmd; print -r -- \"$PROMPT\"",
        )
        .expect("zsh");
        ...
        assert_eq!(
            run(&worktree, "kanso_branch $PWD").as_deref(),
    ...
    fn a_detached_head_is_named_by_its_commit() {
        ...
        let root = std::env::temp_dir().join(format!("kanso-detached-{}", uuid::Uuid::new_v4()));
        let shim = ensure_zsh_shim_in(&root).expect("write the shim");
